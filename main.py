...
使用方式：
    ...
    # 行业深度分析模式
    python main.py --mode industry                      # 分析全部行业
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='''
示例:
  python main.py                              # 默认: 单股分析模式
  python main.py --mode industry              # 行业深度分析模式 (全行业)
  python main.py --mode industry --industry cpo  # 分析指定行业 (CPO)
  ...
  python main.py --market-review              # 仅运行大盘复盘
    ...
    parser.add_argument(
        ...
        default='stock',
        help='运行模式: stock(单股分析, 默认) / industry(行业深度分析)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='行业模式: 指定行业ID (如 cpo, semiconductor_memory 等)。'
             '为空时分析 config/industries.yaml 中所有行业。'
    )
    ...
    parser.add_argument(
        ...
        default='all',
        help='输出格式: json(仅JSON) / html(仅HTML) / all(两者都生成, 默认)'
    )
...
def main() -> int:
    ...
    try:
        ...
        if args.mode == "industry":
            logger.info("=" * 60)
            logger.info("模式: 行业深度分析 (Industry Deep-Dive)")
            logger.info("=" * 60)
            ...
            try:
                ...
                industry_id = getattr(args, 'industry', None)

                if industry_id:
                    logger.info("目标行业: %s", industry_id)
                else:
                    logger.info("目标行业: 全部（从 config/industries.yaml 读取）")

                results = pipeline.run(industry_id=industry_id)

                if results:
                    logger.info("\n===== 行业分析结果摘要 =====")
                    for report in results:
                        industry_info = report.get("industry", {})
                        logger.info(
                            "  [%s] %s | 成分股: %d 只",
                            industry_info.get("id", "?"),
                            industry_info.get("name", "?"),
                            len(industry_info.get("stocks", [])),
                        )

                        output_format = getattr(args, 'format', 'all')
                        industry_id = industry_info.get('id', 'unknown')
                        timestamp = report.get("timestamp", "").replace(":", "-")[:19]

                        try:
                            from pathlib import Path
                            output_dir = Path("data") / "industry_reports"
                            output_dir.mkdir(parents=True, exist_ok=True)

                            # ── Markdown 渲染（用于推送）──
                            report_text = None
                            if output_format in ('all', 'json', 'html'):
                                report_text = pipeline.render_report(
                                    industry=report.get("industry", {}),
                                    macro=report.get("macro_analysis", {}),
                                    stock_analyses=report.get("stock_analyses", []),
                                    cio=report.get("cio_report", {}),
                                )

                            # ── JSON 保存（下游决策用）──
                            if output_format in ('all', 'json'):
                                import json as _json
                                json_path = output_dir / f"industry_{industry_id}_{timestamp}.json"
                            ...
                            if output_format in ('all', 'html'):
                                html_content = pipeline.render_html(
                                    industry=report.get("industry", {}),
                                    macro=report.get("macro_analysis", {}),
                                    stock_analyses=report.get("stock_analyses", []),
                                    cio=report.get("cio_report", {}),
                                ...
                                if html_content:
                                    html_path = output_dir / f"industry_{industry_id}_{timestamp}.html"
                                    with open(html_path, "w", encoding="utf-8") as f:
                                        f.write(html_content)
                                    logger.info("  [HTML] %s", html_path)

                            # ── 推送通知（用 Markdown 文本）──
                            if not args.no_notify and report_text:
                                pipeline.push_report(
                                    industry=report.get("industry", {}),
                                    report_text=report_text,
                                )

                        except Exception as save_err:
                            logger.warning("保存/渲染失败（非致命）: %s", save_err)

                    logger.info("任务执行完成 — 行业分析模式")
                else:
                    logger.warning("行业分析未产生结果。请检查 config/industries.yaml 配置。")
            except ImportError as exc:
                logger.error("行业分析管线模块未就绪: %s。请确保已安装 PyYAML (pip install pyyaml)。", exc)
                return 1
            except Exception as exc:
                logger.exception("行业分析执行失败: %s", exc)
                return 1
            return 0
